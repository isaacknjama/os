...
export class ServiceRegistryService implements OnModuleInit {
  ...
  private readonly services: ServiceDefinition[] = [
    ...
    { name: 'nostr', scopes: [ApiKeyScope.ServiceNostr], envKey: 'NOSTR_API_KEY' },
    { name: 'shares', scopes: [ApiKeyScope.ServiceShares], envKey: 'SHARES_API_KEY' },
    { name: 'solowallet', scopes: [ApiKeyScope.ServiceSolowallet], envKey: 'SOLOWALLET_API_KEY' },
    { name: 'chama', scopes: [ApiKeyScope.ServiceChama], envKey: 'CHAMA_API_KEY' },
    { name: 'notification', scopes: [ApiKeyScope.ServiceNotification], envKey: 'NOTIFICATION_API_KEY' },
    ...
    for (const service of this.services) {
      ...
      if (existingKey) {
        try {
          ...
          this.logger.log(`Verified existing API key for ${service.name} service`);
          continue;
        } catch (error) {
          this.logger.warn(`Invalid API key for ${service.name} service, will create new one`);
        }
      }
      ...
      try {
        ...
        this.logger.log(`Created and stored new API key for ${service.name} service`);
      } catch (error) {
        this.logger.error(`Failed to create API key for ${service.name}: ${error.message}`);
      }
    }
  }
  ...
  async rotateServiceKey(serviceName: string): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`Failed to rotate API key for ${service.name}: ${error.message}`);
      return false;
    }
  }
}